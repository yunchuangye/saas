...
export default function BankDashboardPage() {
  ...
  const statCards = [
    ...
    { title: "待审核报告", value: (stats as any)?.pendingReports ?? 0, icon: FileText, description: "需要尽快处理" },
  ...
  const recentProjects: Project[] = (recentProjectsRaw ?? []).map((p: any) => ({
    ...
    createdAt: new Date(p.createdAt).toISOString().split("T")[0], href: `/dashboard/bank/projects/${p.id}`,
  }));
  const tasks: Task[] = (recentProjectsRaw ?? []).filter((p: any) => ["bidding","reviewing"].includes(p.status)).slice(0,4).map((p: any, i: number) => ({
    ...
    href: `/dashboard/bank/projects/${p.id}`,
  }));
  ...
  return (
    <div className="space-y-6">
      <WelcomeCard userName={user?.displayName ?? "用户"} role="银行机构" quickActions={[
        { label: "发起新评估需求", href: "/dashboard/bank/demand/new" },
        { label: "查看竞价项目", href: "/dashboard/bank/bidding" },
      ]} />
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {statCards.map((stat) => <StatCard key={stat.title} {...stat} />)}
      </div>
      <div className="grid gap-6 lg:grid-cols-2">
        <ActivityChart title="业务趋势" description="近期项目发起和报告完成数量" data={chartData.length>0?chartData:fallback} />
        <TaskList title="待办任务" tasks={tasks} viewAllHref="/dashboard/bank/projects" />
      </div>
      <RecentProjects title="最近项目" projects={recentProjects} viewAllHref="/dashboard/bank/projects" />
    </div>
  );
}
